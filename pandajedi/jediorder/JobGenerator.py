            ...
            try:
                ...
                for tmpJediTaskID, inputList in taskInputList:
                    ...
                    for idxInputList, tmpInputItem in enumerate(inputList):
                        ...
                        self.buildSpecMap = {}
                        loopStart = datetime.datetime.now(datetime.UTC)
                        # make logger
                        tmpLog = MsgWrapper(
                            self.logger,
                        ...
                        if goForward:
                            ...
                            if (
                                ...
                                and datetime.datetime.now(datetime.UTC) - taskSpec.frozenTime > datetime.timedelta(days=7)
                            ):
                        ...
                        if idxInputList + 1 == len(inputList):
                            ...
                            taskSpec.lockedTime = datetime.datetime.now(datetime.UTC)
                        # update task
                        ...
                        tmpLog.sendMsg(tmpMsg, self.msgType)
                        tmpLog.info(tmpMsg)
                        regTime = datetime.datetime.now(datetime.UTC) - loopStart
    ...
    def doGenerateBuild(self, taskSpec, cloudName, siteName, siteSpec, taskParamMap, tmpLog, simul=False):
        ...
        try:
            ...
            if fileSpec is not None:
                if fileSpec.creationDate > datetime.datetime.now(datetime.UTC) - datetime.timedelta(days=periodToUselibTgz):
            ...
            if (
                ...
                or datasetSpec.creationTime < datetime.datetime.now(datetime.UTC) - datetime.timedelta(days=periodToUselibTgz)
            ):
                # make new libDS
                reusedDatasetID = None
                libDsName = (
                    f"panda.{time.strftime('%m%d%H%M%S', time.gmtime())}.{datetime.datetime.now(datetime.UTC).microsecond}.lib._{jobSpec.jediTaskID:06d}"
                )
            else:
                # reuse existing DS
