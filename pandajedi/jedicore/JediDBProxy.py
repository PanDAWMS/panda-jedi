...
class DBProxy(taskbuffer.OraDBProxy.DBProxy):
    ...
    def reactivatePendingTasks_JEDI(self,vo,prodSourceLabel,timeLimit,timeoutLimit=None,minPriority=None):
        ...
        try:
            ...
            for jediTaskID,frozenTime,errorDialog,parent_tid,splitRule,startTime in resTL:
                ...
                self.cur.execute(sql+comment,varMap)
                if timeoutFlag:
                    tmpLog.info('jediTaskID={0} timeout #ATM'.format(jediTaskID))
                elif keepFlag:
                    tmpLog.info('jediTaskID={0} keep pending #ATM'.format(jediTaskID))
                else:
                    tmpLog.info('jediTaskID={0} reactivated #ATM'.format(jediTaskID))
                nRow += self.cur.rowcount
    ...
    def releaseThrottledTasks_JEDI(self,vo,prodSourceLabel):
        ...
        try:
            ...
            for jediTaskID,oldStatus in resTL:
                ...
                tmpLog.info('released jediTaskID={0} with {1} #ATM'.format(jediTaskID,iRow))
                nRow += iRow
    ...
    def getTaskWithJumbo_JEDI(self, vo, prodSourceLabel):
        ...
        try:
            ...
            sqlFR = "SELECT /*+ NO_INDEX_FFS(tab JEDI_EVENTS_PK) NO_INDEX_SS(tab JEDI_EVENTS_PK) NO_INDEX(tab JEDI_EVENTS_PANDAID_STATUS_IDX)*/ "
