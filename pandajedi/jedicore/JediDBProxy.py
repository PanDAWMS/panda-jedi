...
class DBProxy(OraDBProxy.DBProxy):
    ...
    def refreshWorkQueueMap(self):
        ...
        if self.updateTimeForWorkQueue is not None and (datetime.datetime.now(datetime.timezone.utc) - self.updateTimeForWorkQueue) < datetime.timedelta(
            minutes=10
        ):
            return
        ...
        try:
            ...
            tmpLog.debug("done")
            self.updateTimeForWorkQueue = datetime.datetime.now(datetime.timezone.utc)
            return True
        except Exception:
            # roll back
            self._rollback()
    ...
    def getDatasetsToFeedContents_JEDI(self, vo, prodSourceLabel, task_id=None):
        ...
        try:
            ...
            if task_id is None:
                ...
                checkInterval = 0
            varMap[":checkTimeLimit"] = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=checkInterval)
            varMap[":lockTimeLimit"] = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=10)
            ...
            if res is None:
                ...
                if lockedBy != pid:
                    ...
                    if taskSpec is not None:
                        ...
                        varMap[":updateTime"] = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(hours=6)
                    else:
            ...
            if nRows > 0 and insertUnknown is not None:
                ...
                for tmpUnknownDataset in insertUnknown:
                    ...
                    if resUC is None:
                        ...
                        datasetSpec.creationTime = datetime.datetime.now(datetime.timezone.utc)
    ...
    def getTaskWithID_JEDI(self, jediTaskID, fullFlag, lockTask=False, pid=None, lockInterval=None, clearError=False):
        ...
        try:
            ...
            varMap[":jediTaskID"] = jediTaskID
            if lockInterval is not None:
                varMap[":timeLimit"] = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=lockInterval)
            # begin transaction
            self.conn.begin()
            # select
            res = None
    ...
    def getTasksToBeFinished_JEDI(self, vo, prodSourceLabel, pid, nTasks=50, target_tasks=None):
        ...
        try:
            ...
            self.cur.arraysize = 10000
            # get tasks
            timeLimit = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=10)
    ...
    def getTasksToBeProcessed_JEDI(
        ...
        methodName = self.getMethodName(comment)
        timeNow = datetime.datetime.now(datetime.timezone.utc).strftime("%Y/%m/%d %H:%M:%S")
        ...
        if hasattr(jedi_config.jobgen, "lockInterval"):
            ...
            lockInterval = 10
        timeLimit = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=lockInterval)
        try:
                    ...
                    if origTaskSpec.is_hpo_workflow():
                        ...
                        if not [i for i in numEventsHPO.values() if i > 0]:
                            ...
                            if totalNumEventsHPO + totalNumWorkersHPO == 0 and origTaskSpec.frozenTime is None:
                                varMap = {}
                                varMap[":jediTaskID"] = jediTaskID
                                varMap[":frozenTime"] = datetime.datetime.now(datetime.timezone.utc)
                            ...
                            if (
                                ...
                                and datetime.datetime.now(datetime.timezone.utc) - origTaskSpec.frozenTime > datetime.timedelta(hours=waitInterval)
                            ):
                                # send finish command
    ...
    def rescuePickedFiles_JEDI(self, vo, prodSourceLabel, waitTime):
        ...
        try:
            ...
            self.conn.begin()
            self.cur.arraysize = 10000
            timeLimit = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=waitTime)
            # get orphaned tasks
            varMap = {}
    ...
    def rescueUnLockedTasksWithPicked_JEDI(self, vo, prodSourceLabel, waitTime, pid):
        ...
        tmpLog.debug("start")
        try:
            timeToCheck = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=waitTime)
            varMap = {}
            ...
            for jediTaskID, datasetIDs in taskDsMap.items():
                ...
                if not toSkip:
                    ...
                    if allOK:
                        ...
                        varMap[":lockedBy"] = pid
                        varMap[":lockedTime"] = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(hours=24)
    ...
    def getTasksToRefine_JEDI(self, vo=None, prodSourceLabel=None):
        ...
        try:
            ...
            varMap[":timeLimit"] = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=10)
    ...
    def setScoutJobDataToTasks_JEDI(self, vo, prodSourceLabel, site_mapper):
        ...
        try:
            ...
            varMap[":status"] = "running"
            varMap[":minJobs"] = 5
            varMap[":timeLimit"] = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(hours=24)
    ...
    def prepareTasksToBeFinished_JEDI(self, vo, prodSourceLabel, nTasks=50, simTasks=None, pid="lock", noBroken=False, site_mapper=None):
        ...
        try:
            ...
            if simTasks is None:
                minSuccessScouts = 5
                timeToCheck = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=10)
                varMap = {}
    ...
    def getTasksToAssign_JEDI(self, vo, prodSourceLabel, workQueue, resource_name):
        ...
        try:
            ...
            varMap[":timeLimit"] = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=30)
    ...
    def getTasksToExecCommand_JEDI(self, vo, prodSourceLabel):
        ...
        try:
            ...
            for commandStr, taskStatusMap in commandStatusMap.items():
                ...
                self.conn.begin()
                # FIXME
                # varMap[':timeLimit'] = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(hours=1)
                varMap[":timeLimit"] = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=5)
    ...
    def getArchView(self, timeStamp):
        ...
        timeDelta = datetime.datetime.now(datetime.timezone.utc) - timeStamp
    ...
    def reactivatePendingTasks_JEDI(self, vo, prodSourceLabel, timeLimit, timeoutLimit=None, minPriority=None):
        ...
        try:
            ...
            if timeoutLimit is not None:
                timeoutDate = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(hours=timeoutLimit)
            # sql to get pending tasks
            varMap = {}
            varMap[":status"] = "pending"
            varMap[":timeLimit"] = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=timeLimit)
    ...
    def kickExhaustedTasks_JEDI(self, vo, prodSourceLabel, timeLimit):
        ...
        try:
            ...
            varMap[":timeLimit"] = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(hours=timeLimit)
        ...
        try:
            ...
            if resDS is None:
                ...
                if resFR is not None:
                    ...
                    tmpFileSpec.creationDate = datetime.datetime.now(datetime.timezone.utc)
        ...
        try:
            ...
            if resTK is None:
                ...
                if newTaskStatus != taskOldStatus:
                    ...
                    varMap[":updateTime"] = datetime.datetime.now(datetime.timezone.utc)
    ...
    def getTasksWithCriteria_JEDI(
        ...
        try:
            ...
            timeLimit = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=taskLockInterval)
    ...
    def getLibForWaitingRunJob_JEDI(self, vo, prodSourceLabel, checkInterval):
        ...
        try:
            ...
            varMap[":timeLimit"] = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=checkInterval)
            self.cur.execute(sqlL + comment, varMap)
    ...
    def kickChildTasks_JEDI(self, jediTaskID):
        ...
        try:
            ...
            timeLimitT = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=5)
            sqlCT = f"UPDATE {jedi_config.db.schemaJEDI}.JEDI_Tasks "
            ...
            timeLimitD = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=5)
    ...
    def throttleTasks_JEDI(self, vo, prodSourceLabel, waitTime):
        ...
        try:
            ...
            for jediTaskID, numThrottled, largestAttemptNr in resList:
                ...
                try:
                    numThrottled += 1
                    throttledTime = datetime.datetime.now(datetime.timezone.utc)
    ...
    def throttleTask_JEDI(self, jediTaskID, waitTime, errorDialog):
        ...
        try:
            ...
            varMap[":newStatus"] = "throttled"
            varMap[":oldStatus"] = "running"
            varMap[":releaseTime"] = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(minutes=waitTime)
            varMap[":errorDialog"] = errorDialog
            self.cur.execute(sqlTH + comment, varMap)
            nRow = self.cur.rowcount
            tmpLog.debug(f"done with {nRow}")
    ...
    def getAchievedTasks_JEDI(self, vo, prodSourceLabel, timeLimit, nTasks):
        ...
        try:
            ...
            self.cur.arraysize = 10000
            # get tasks
            timeToCheck = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=timeLimit)
            varMap[":timeLimit"] = timeToCheck
            tmpLog.debug(sqlRT + comment + str(varMap))
            self.cur.execute(sqlRT + comment, varMap)
    ...
    def getInactiveSites_JEDI(self, flag, timeLimit):
        ...
        try:
            ...
            varMap[":flag"] = flag
            varMap[":hours"] = 3
            varMap[":laststart"] = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(hours=timeLimit)
            self.cur.execute(sqlCD + comment, varMap)
    ...
    def getNetworkMetrics(self, dst, keyList):
        ...
        latest_validity = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=60)
        ...
        try:
            ...
            for table in ["jobsActive4", "jobsArchived4"]:
                ...
                if table == "ATLAS_PANDA.jobsActive4":
                    ...
                    varMap[":modificationTime"] = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=60)
                    sqlJ = (
                        "SELECT COUNT(*),prodUserName,jobStatus,workingGroup,computingSite,coreCount "
                        "FROM {0}.{1} "
    ...
    def get_pending_dc_tasks_JEDI(self, task_type="prod", time_limit_minutes=60):
        ...
        try:
            ...
            var_map[":timeLimit"] = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=time_limit_minutes)
