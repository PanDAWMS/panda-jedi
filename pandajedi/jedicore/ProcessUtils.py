            ...
            while True:
                try:
                    ...
                    if not JediCoreUtils.checkProcess(pid):
                        ...
                        print("{0} {1}: INFO    pid={2} not exist".format(str(timeNow), self.__class__.__name__, pid))
                        break
                except Exception:
                    timeNow = datetime.datetime.utcnow()
                    errType, errValue = sys.exc_info()[:2]
                    print("{0} {1}: INFO    failed to check pid={2} with {3} {4}".format(str(timeNow), self.__class__.__name__, pid, errType, errValue))
