...
class AtlasAnalPostProcessor(PostProcessorBase):
    ...
    def doPostProcess(self, taskSpec, tmp_logger):
        ...
        try:
            ...
            use_lib = False
            n_ok_lib = 0
            lock_update_time = datetime.datetime.now(datetime.timezone.utc)
            for datasetSpec in taskSpec.datasetSpecList:
                ...
                datasetSpec.stateCheckTime = datetime.datetime.now(datetime.timezone.utc)
                ...
                # update task lock
                if datetime.datetime.now(datetime.timezone.utc) - lock_update_time > datetime.timedelta(minutes=5):
                    lock_update_time = datetime.datetime.now(datetime.timezone.utc)
                    # update lock
    ...
    def getEmail(self, user_name, vo, tmp_logger):
        ...
        if dn in ["", None]:
            ...
            if db_uptime is not None and datetime.datetime.now(datetime.timezone.utc) - db_uptime < datetime.timedelta(hours=1):
