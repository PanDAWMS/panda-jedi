...
def daemon_loop(dem_config, msg_queue, pipe_conn, worker_lifetime, tbuf, ddmif):
    # pid of the worker
    my_pid = os.getpid()
    ...
    mod_package = getattr(jedi_config.daemon, 'package', 'pandajedi.jedidaemons.daemon_scripts')
    # start timestamp
    start_ts = time.time()
    # expiry time
            ...
            if to_run_daemon:
                ...
                try:
                    if is_loop:
                        ...
                        start_ts = time.time()
                        while True:
                            ret_val = the_module.main(argv=mod_argv, tbuf=tbuf, ddmif=ddmif)
                            now_ts = time.time()
                            if not ret_val:
                        ...
                        the_module.main(argv=mod_argv, tbuf=tbuf, ddmif=ddmif)
...
class DaemonWorker(object):
    ...
    # constructor
    def __init__(self, dem_config, msg_queue, worker_lifetime, tbuf, ddmif):
    ...
    def _make_process(self, dem_config, msg_queue, worker_lifetime, tbuf, ddmif):
        args = (dem_config, msg_queue, self.child_conn, worker_lifetime, tbuf, ddmif)
...
class DaemonMaster(object):
    ...
    # constructor
    def __init__(self, logger, tbuf, ddmif, n_workers=1, worker_lifetime=28800):
        # logger
        self.logger = logger
    ...
    def _parse_config(self):
        try:
            config_json = jedi_config.daemon.config
