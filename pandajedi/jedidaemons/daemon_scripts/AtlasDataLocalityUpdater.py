...
class AtlasDataLocalityUpdater(object):

    # constructor
    def __init__(self, taskBufferIF, ddmIF):
        # WatchDogBase.__init__(self, taskBufferIF, ddmIF)
        self.pid = '{0}-{1}-daemon'.format(socket.getfqdn().split('.')[0], os.getpid())
        self.vo = 'atlas'
    ...
    def doUpdateDataLocality(self):
        ...
        try:
            ...
            got_lock = self.taskBufferIF.lockProcess_JEDI(  vo=self.vo, prodSourceLabel='default',
                                                            ...
                                                            component='AtlasDataLocalityUpdater.doUpdateDataLocality',
    ...
    def doCleanDataLocality(self):
        ...
        try:
            ...
            got_lock = self.taskBufferIF.lockProcess_JEDI(  vo=self.vo, prodSourceLabel='default',
                                                            cloud=None, workqueue_id=None, resource_name=None,
                                                            component='AtlasDataLocalityUpdater.doCleanDataLocality',
...
def main(argv=tuple(), tbuf=None, ddmif=None):
    ...
    try:
        # instantiate
        daemon_obj = AtlasDataLocalityUpdater(taskBufferIF=tbuf, ddmIF=ddmif)
        # clean up data locality
        daemon_obj.doCleanDataLocality()
        # update data locality
        daemon_obj.doUpdateDataLocality()
    except Exception:
        errtype, errvalue = sys.exc_info()[:2]
