        ...
        while True:
            try:
                ...
                for item in taskDsList:
                    ...
                    is_container = datasetName.endswith("/")
                    for tmpRSE, tmpList in dataset_replicas_map.items():
                        tmpStatistics = tmpList[-1]
                        # skip unknown and incomplete datasets (while no check for input container)
                        if not is_container and (tmpStatistics["found"] is None or tmpStatistics["found"] != tmpStatistics["total"]):
